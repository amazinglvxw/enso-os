...
- Bad: '- Exit code 1' or '- File not found' (these are symptoms not lessons)
- Good: '- Use offset/limit when reading files over 5000 lines to avoid token overflow'
Only output lessons. No preamble." 2>/dev/null) || true
fi
...
while IFS= read -r lesson; do
    ...
    # Semantic dedup: extract keywords, check if any existing lesson shares 60%+
    IS_DUP=$(python3 -c "
import re, sys
